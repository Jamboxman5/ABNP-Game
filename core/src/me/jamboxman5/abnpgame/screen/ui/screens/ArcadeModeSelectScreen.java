...
public class ArcadeModeSelectScreen implements Screen {
...
//        Fonts.drawScaled(Fonts.SUBTITLEFONT, .841f, subTitle, batch, x, y+ Fonts.getTextHeight(title, Fonts.SUBTITLEFONT, 1f));
    ...
    private void getButtons() {
        ...
        int x = (int) (alignX - Fonts.getTextWidth("Singleplayer", Fonts.BUTTONFONT, 1f));
        int y = spacer*(buttons.length-1);
        singlePlayer = new Button(x, y, "Singleplayer", Fonts.BUTTONFONT, Button.TextAlign.RIGHT);
        x = (int) (alignX - Fonts.getTextWidth("Multiplayer", Fonts.BUTTONFONT, 1f));
        y -= spacer;
        multiPlayer = new Button(x, y, "Multiplayer", Fonts.BUTTONFONT, Button.TextAlign.RIGHT);
        ...
        buttons[2] = multiPlayer;
                ...
                singlePlayer.setAction(new Runnable() {
                    @Override
                    public void run() {
                        Screen old = game.getScreen();
        ...
        multiPlayer.setAction(new Runnable() {
            @Override
            public void run() {
                Screen old = game.getScreen();
                game.setScreen(new MultiplayerModeSelectScreen(game));
                old.dispose();
            }
        });
