...
public class PistolDesertEagle extends Firearm {
	...
	public PistolDesertEagle() {
		this(new WeaponModLoadout(), new StandardAmmo(), 8, UUID.randomUUID().toString());
	}

	public PistolDesertEagle(WeaponModLoadout mods, Ammo ammo, int loadedAmmo, String id) {
		...
		this.type = WeaponType.DESERTEAGLE;
		this.xOffset = 0;
		this.yOffset = 14;
	...
	public static void loadAssets(AssetManager assets) {
		// Sprites (Textures)
		assets.load("weapon/pistol/DesertEagle.png", Texture.class);
	...
	public static void loadSprites(AssetManager assets) {
		hudSprite = setup("weapon/pistol/DesertEagle.png", assets, .1f);
