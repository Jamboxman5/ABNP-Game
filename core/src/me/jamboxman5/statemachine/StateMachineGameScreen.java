...
import me.jamboxman5.galaga.GalagaGameOverScreen;
...
public class StateMachineGameScreen implements Screen {
    ...
    public StateMachineGameScreen(final StateMachineGame game) {
        this.game = game;
    ...
    private class Bullet {
        ...
        public boolean overlaps(Rectangle rect) { return new Rectangle(x, y, 64,64).overlaps(rect); }
        public void update() {
    ...
    void enemyAttack() {
        if (attacker != null) {
            ...
            attacker.y -= 650*getSpeedModifier();
        }
        ...
        if (attacker.y > 250) attackPosition = new Vector2(player.x, player.y);
    ...
    double getSpeedModifier() { return (1.0+(level/15.0)) / 100.0; }
    double getBulletSpeedModifier() { return 1.3*(1.0+(level/15.0)) / 80.0; }
    boolean retarget() { return ((Math.random()*10) + level > 15); }
    void renderStars() {
    ...
    boolean isDead() { return (System.currentTimeMillis() - lastDeath < 2000);}
    private void die() {
        lives--;
        deathSound.play();
        player.x = 400-32;
        lastDeath = System.currentTimeMillis();
        if (lives < 0) game.setScreen(new StateMachineMenuScreen(game));
    }
    private void shoot() {
        if (isDead()) return;
