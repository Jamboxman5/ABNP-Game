...
public class LegacyStartMenuScreen implements Screen {
    ...
    public LegacyStartMenuScreen(final LegacyGame game) {
        this.game = game;
    ...
    void renderStars() {
        ...
        if (System.currentTimeMillis() - lastNewStar > 50) {
            ...
            int newX = (int) (Math.random()*game.WIDTH);
            int newY = (int) (Math.random()*game.HEIGHT);
    ...
    public void render(float delta) {
        ...
        shapeRenderer.end();
        game.batch.begin();
        game.font.draw(game.batch, "Legacy of Goku III", 100, 150);
        game.font.draw(game.batch, "Click Anywhere to Start!", 100, 100);
        game.batch.end();

        if (Gdx.input.isTouched()) {
            game.setScreen(new LegacyGameScreen(game));
            dispose();
        }
